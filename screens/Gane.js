...
class Game extends React.Component {
  ...
  constructor(props) {
    ...
    this.storage
      .getStats(navigation.getParam("aiLevel", 3).toString())
      .then((stats) => {
        this.stats = stats;
      })
      .catch(console.error);
  ...
  onPress = (i, j, process) => {
    ...
    if (
      (points[i][j] && (points[i][j] === "o" || points[i][j] === "x")) ||
      disable
    )
      return;
    ...
    if (pozitionsLeft === 8 && this.state.max === 3 && this.state.ai) {
      setTimeout(() => {
        if (!this.end) this.computerTurn(i, j);
      }, 300);
    } else {
      if (this.state.pvp) disable = this.end;
      else if (!this.end) this.computerTurn(i, j);
    }
  };
  ...
  setStats(winner) {
    ...
    this.stats[winner] += 1;
    this.storage
      .setStats(this.state.aiLevel.toString(), this.stats)
      .catch(console.error);
  }
  ...
  checkGameStatus(i, j, turn) {
    if (
      ...
      this.checkHorizontalVertical(i, j) ||
      this.checkVerticalHorizontal(i, j)
    ) {
      winPositions.forEach((item) => {
        refs[item.i][item.j].current.onShowWin(points[item.i][item.j]);
      });
      setTimeout(() => {
        this.modalWinner.current.onPress(points[i][j]);
      }, 3000);
      this.setStats(turn);
      return true;
  ...
  checkVertical(i, j) {
    ...
    var z = i - this.state.numberCharsToWin,
      index = i + this.state.numberCharsToWin;
    for (z; z < index; z++) {
  ...
  checkHorizontal(i, j) {
    let status = { x: 0, o: 0 };
    var player = this.x ? "x" : "o";
    var z = j - this.state.numberCharsToWin,
      index = j + this.state.numberCharsToWin;
  ...
  checkHorizontalVertical(i, j) {
    let status = { x: 0, o: 0 };
    var player = this.x ? "x" : "o";
    var y = i - this.state.numberCharsToWin,
      indexy = i + this.state.numberCharsToWin;
    var z = j - this.state.numberCharsToWin,
      index = j + this.state.numberCharsToWin;

    for (z, y; z < index, y < indexy; z++, y++) {
      if (z < 0 || y < 0 || z >= this.state.max || y >= this.state.max)
        continue;
  ...
  getScrollView() {
    ...
    if (this.state.ads) {
      return (
        <View
          style={[styles.loading, { backgroundColor: theme.backgroundColor }]}
        >
          <ActivityIndicator
          ...
          >
            <View
              style={{
                ...
                padding: 10,
                paddingBottom: 50,
                paddingTop: this.state.ai ? 75 : 150,
              }}
            >
              {this.state.ai ? (
                <StatsComponent
  ...
  zoomIn() {
    ...
    if (this.state.zoomLevel < 3)
      this.setState((oldState) => {
        return {
          squareLength: this.state.squareLength + zoom,
          zoomLevel: (oldState.zoomLevel += 1),
        };
      });
  }
  ...
  render() {
    ...
    return (
      <View style={{ height: layout.window.height }}>
        <NavigationEvents onWillFocus={() => this.create()} />
        <View opacity={this.state.ads ? 0 : 0.35} style={styles.pauseButton}>
          <FontAwesome.Button
            name="pause-circle-o"
            backgroundColor={backgroundColor}
            onPress={() => this.setModalVisiblePause(true)}
            size={35}
            color={playButton}
          ></FontAwesome.Button>
        </View>
        <View
          opacity={this.state.ads ? 0 : this.state.zoomLevel > 0 ? 0.5 : 0}
          style={[styles.zoomButtonsMinus, { backgroundColor }]}
        >
          <FontAwesome.Button
            name="minus-square-o"
            backgroundColor={backgroundColor}
        ...
        <View
          opacity={this.state.ads ? 0 : this.state.zoomLevel < 3 ? 0.5 : 0}
          style={[styles.zoomButtons, { backgroundColor }]}
        >
          <FontAwesome.Button
            name="plus-square-o"
            backgroundColor={backgroundColor}
